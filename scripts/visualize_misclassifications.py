...
def visualize_misclassifications(csv_path, n_images=9, cols=3, output_path=None):
    df = pd.read_csv(csv_path)
    # Only keep misclassified samples
    misclassified = df[df['true_label'] != df['predicted_label']]
    N = min(n_images, len(misclassified))
    if N == 0:
        print("No misclassifications found!")
        return
    sample = misclassified.sample(N, random_state=42)
    rows = (N + cols - 1) // cols
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize misclassified predictions as an image grid.")
    ...
    visualize_misclassifications(args.csv_path, args.n_images, args.cols, args.output_path) 